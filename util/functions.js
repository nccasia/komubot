...
const checkConfig = async (config) => {
  ...
  if (
    ...
    (config.defaultLanguage.toLowerCase() !== 'fr' &&
      config.defaultLanguage.toLowerCase() !== 'en')
  ) {
    console.error(
      '✗ The default Language parameter is missing or is not supported. Langages: fr, en, de'
    );
    error = true;
  }
  if (!config.devMode || typeof config.devMode !== Boolean) {
    console.error(
      '✗ The devMode parameter is missing or is not a bolean value'
    );
    error = true;
  }
  if (!config.devServer || config.devServer.length !== 18) {
    if (!config.devMode) {
      console.error(
        '✗ The devServer parameter is missing or is not a valid discord ID'
      );
      error = true;
    }
  }
  ...
  if (!config.token) {
    console.error(
      '✗ Please provide a discord bot token.get it at https://discord.com/developers/bots'
    );
    error = true;
  } else {
  ...
  if (!config.database.MongoURL) {
    console.error(
      '✗ Please provide the url of your mongodb database.Your can get it at https://mongodb.org'
    );
    error = true;
  } else {
